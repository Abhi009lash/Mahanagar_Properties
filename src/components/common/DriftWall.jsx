...
const DriftWall = ({
  ...
  const columnItems = useMemo(() => {
    ...
    for (let c = 0; c < columns; c++) {
      const colArr = [...items];
      // Deterministic shuffle per column for consistent randomized visual mix
      for (let i = colArr.length - 1; i > 0; i--) {
        const j = Math.floor(((c * 17 + i * 31 + 7) % (i + 1)));
        [colArr[i], colArr[j]] = [colArr[j], colArr[i]];
      }
      const offset = (c * 3) % colArr.length;
      const shifted = [...colArr.slice(offset), ...colArr.slice(0, offset)];
      cols.push(shifted);
    }
    return cols;
  }, [items, columns]);
  ...
  const renderTile = (item, id, colIndex) => {
    const inner = (
      <span className="drift-wall__inner">
        ...
        {item.title && (
          <span className="drift-wall__title-badge">
            {item.title}
          </span>
        )}
      </span>
